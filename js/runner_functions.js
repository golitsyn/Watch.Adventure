...
function send_party_invite(name, is_request) // name could be a player object, name, or id
{
  ...
  parent.socket.emit('party', {
    event: is_request && 'request' || 'invite',
    name: name
  });
}
...
function smart_move(destination, on_done) // despite the name, smart_move isn't very smart or efficient, it's up to the players to implement a better movement method [05/02/17]
{
  ...
  if ("x" in destination) {
    ...
    if (G.monsters[destination.to]) {
      for (var name in G.maps)(G.maps[name].monsters || []).forEach(function(pack) {
...
function bfs() {
  ...
  while (start < queue.length) {
    ...
    moves.forEach(function(m) {
      var new_x = parseInt(current.x + m[0]),
        new_y = parseInt(current.y + m[1]);
...
function smart_move_logic() {
  ...
  if (!smart.searching && !smart.found) {
    ...
    if (Math.random() < 0.1) {
      ...
      parent.d_text(shuffle(["Hmm", "...", "???", "Definitely left", "No right!", "Is it?", "I can do this!", "I think ...", "What If", "Should be", "I'm Sure", "Nope", "Wait a min!", "Oh my"])[0], character, {
