...
function start_runner(a, b) {
  ...
  actual_code = false;
  if (b === undefined) {
    b = codemirror_render.getValue(), actual_code = true
  }
  the_code = b;
  $(".engagebutton").hide();
  $(".dengagebutton").show();
...
function h_shake() {
  ...
  var a = 0;[-1, 1, -2, 2, -3, 3, -3, 3, -3, 3, -2, 2, -1, 1].forEach(function(c) {
    setTimeout(b(c), a++ * 80)
  })
}
function bump_up(a, c) {
  ...
  var b = 0;[1, 1, 1, -0.5, -0.5, -0.5, -0.5, -0.5, -0.5].forEach(function(f) {
...
function add_border(f, g, d) {
  if (!g) {
    g = (f.texture.width), d = (f.texture.height)
  }
